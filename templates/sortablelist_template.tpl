{* sortablelist_template.tpl - v1.2
 - v1.2 - 01May22 - add block-specific ul class, remove 'delete' links
 - v1.1 - 15Feb19 - removed output of js & css - now module wide on first call
 - v1.0 - 18Apr17 - 1st version
*}
<input type="hidden" id="{$selectarea_prefix}" name="{$selectarea_prefix}" value="{$selected_str}" />
<div class="c_full cf ecb2-cb {$selectarea_prefix}">
  <div class="grid_6 alpha">
    <fieldset>
      ...
      <ul class="ecb2-sortable-list {$detail} selected-items" data-cmsms-cb-input="{$selectarea_prefix}"
      {if $maxNumber} data-max-number="{$maxNumber}"{/if}{if $requiredNumber} data-required-number="{$requiredNumber}"{/if}>
        ...
        {foreach $selected as $alias => $name}
          <li class="ui-state-default drag-item" data-cmsms-item-id="{$alias}">
            {$name}
{*          <a href="#" title="{$mod->Lang('remove')}" class="ui-icon ui-icon-trash sortable-remove">{$mod->Lang('remove')}</a>*}
  ...
  <div class="grid_6 omega draggable-area">
    <fieldset>
      ...
      <ul class="ecb2-sortable-list {$detail} available-items">
